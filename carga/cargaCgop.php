...
try {
	...
	$DadosContrato = [];
	$p_sql = connection::open('sindnit')->prepare("SELECT * FROM Dados_Contrato Where (DS_MODAL in ('HIDROVIARIO', 'PORTUARIO') OR U_CON_FORMATADO = '10 00560/2021') and DS_TIP_CONTRATO like '%OBRA DE ENGENHARIA%'");
	$p_sql->execute();
	//-------------------------------------------------------------------
...
try {
	$DadosContratosp = [];
	$psp_sql = connection::open('sindnit')->prepare("SELECT * FROM Dados_Contrato Where DS_MODAL in ('HIDROVIARIO', 'PORTUARIO') and DS_TIP_CONTRATO like '%SUPERVISAO%'");
	$psp_sql->execute();
	//-------------------------------------------------------------------
...
try {
	$query = connection::open('supradaq')->prepare( "SELECT * FROM CGOB_TB_CONTRATO_OBRA" );
	$query->execute();
	//-------------------------------------------------------------------
...
try {
	$query_sp = connection::open('supradaq')->prepare( "SELECT * FROM CGOB_TB_CONTRATO_SUPERVISORA" );
	$query_sp->execute();
	//-------------------------------------------------------------------
...
try {
	$sqlk = "UPDATE CGOB_TB_CONTRATO_OBRA SET  nu_cnpj_cpf = :nu_cnpj_cpf
	...
	$sqlup = connection::open('supradaq')->prepare("$sqlk");
	$qtdUpdObra = 0;
	...
	if($exception){
		...
		print("Update $qtdUpdObra rows CGOB_TB_CONTRATO_OBRA!<br/>");
	}
...
try {
	...
	if(!empty($DadosContratoObrainst)){
		foreach ($DadosContratoObrainst as $value):
			...
			$exceptionins = $insert_obra->execute();
			$qtdInsObra++;
		endforeach;
	}
	if($exceptionins){
		//$count = $insert_obra->rowCount();
		print("Insert $qtdInsObra rows CGOB_TB_CONTRATO_OBRA!<br/>");
	}
} catch (PDOException $e) {
...
try {
	$sqlk = "UPDATE CGOB_TB_CONTRATO_SUPERVISORA SET  nu_cnpj_cpf = :nu_cnpj_cpf
			,sk_contrato = :sk_contrato
			,nu_con_formatado = :nu_con_formatado
			,sk_contrato_supervisor = :sk_contrato_supervisor
			...
			,Valor_Oficio_Pagamento = :Valor_Oficio_Pagamento
			WHERE nu_con_formatado = :num";
	$sql_sp = connection::open('supradaq')->prepare("$sqlk");
	$qtdUpdSup = 0;
	...
	if($exception){
		...
		print("Update $qtdUpdSup rows CGOB_TB_CONTRATO_SUPERVISORA!<br/>");
	}
} catch (PDOException $e) {
...
$exceptionsup = false;
try {
	$insert_sup = connection::open('supradaq')->prepare( "INSERT INTO CGOB_TB_CONTRATO_SUPERVISORA ( nu_cnpj_cpf,sk_contrato,nu_con_formatado,sk_contrato_supervisor,nu_con_formatado_supervisor,dt_base,dt_corrente,dt_termino_vigencia,dt_aprovacao,dt_assinatura,dt_proposta,dt_publicacao,dt_inicio,dt_ter_atz,dt_ter_prv,sk_empresa,no_empresa,sk_empresa_supervisor,sg_empresa_supervisor,sk_fiscal,nm_fiscal,ds_grupo_intervencao,sk_modal,ds_modal,modalidade_licitacao,sk_municipio,no_municipio,co_municipio,no_municipio0,nu_edital,nu_lote_licitacao,nu_processo,ds_objeto,sk_programa,nm_programa,nu_dia_paralisacao,nu_dia_prorrogacao,sk_situacao_contrato,ds_fas_contrato,co_tip_contrato,ds_tip_contrato,sk_tipo_intervencao,ds_tip_intervencao,tipo_licitacao,descricao_br,sk_uf_unidade_local,sg_uf_unidade_local,co_uf,sg_uf,sk_unidade_fiscal,nm_und_fiscal,sg_und_fiscal,sk_unidade_gestora,nm_und_gestora,sg_und_gestora,sk_unidade_local,nm_und_local,sg_und_local,sk_unidade_pagamento,nm_und_pagamento,sg_und_pagamento,Extensao_Total,Valor_Inicial,Valor_Total_de_Aditivos,Valor_Total_de_Reajuste,Valor_Inicial_Adit_Reajustes,Valor_Empenhado,Valor_Saldo,Valor_Medicao_PI_R,Valor_PI_Medicao,Valor_Reajuste_Medicao,Valor_Oficio_Pagamento)  
				VALUES (:nu_cnpj_cpf,:sk_contrato,:nu_con_formatado,:sk_contrato_supervisor,:nu_con_formatado_supervisor,:dt_base,:dt_corrente,:dt_termino_vigencia,:dt_aprovacao,:dt_assinatura,:dt_proposta,:dt_publicacao,:dt_inicio,:dt_ter_atz,:dt_ter_prv,:sk_empresa,:no_empresa,:sk_empresa_supervisor,:sg_empresa_supervisor,:sk_fiscal,:nm_fiscal,:ds_grupo_intervencao,:sk_modal,:ds_modal,:modalidade_licitacao,:sk_municipio,:no_municipio,:co_municipio,:no_municipio0,:nu_edital,:nu_lote_licitacao,:nu_processo,:ds_objeto,:sk_programa,:nm_programa,:nu_dia_paralisacao,:nu_dia_prorrogacao,:sk_situacao_contrato,:ds_fas_contrato,:co_tip_contrato,:ds_tip_contrato,:sk_tipo_intervencao,:ds_tip_intervencao,:tipo_licitacao,:descricao_br,:sk_uf_unidade_local,:sg_uf_unidade_local,:co_uf,:sg_uf,:sk_unidade_fiscal,:nm_und_fiscal,:sg_und_fiscal,:sk_unidade_gestora,:nm_und_gestora,:sg_und_gestora,:sk_unidade_local,:nm_und_local,:sg_und_local,:sk_unidade_pagamento,:nm_und_pagamento,:sg_und_pagamento,:Extensao_Total,:Valor_Inicial,:Valor_Total_de_Aditivos,:Valor_Total_de_Reajuste,:Valor_Inicial_Adit_Reajustes,:Valor_Empenhado,:Valor_Saldo,:Valor_Medicao_PI_R,:Valor_PI_Medicao,:Valor_Reajuste_Medicao,:Valor_Oficio_Pagamento)" );
	$qtdInsSup = 0;
	if(!empty($DadosContratoSupinst)){
		foreach ($DadosContratoSupinst as $value):
			...
			$exceptionsup = $insert_sup->execute();
			$qtdInsSup++;
		endforeach;
	}
	if($exceptionsup){
		//$count = $insert_sup->rowCount();
		print("Insert $qtdInsSup rows CGOB_TB_CONTRATO_SUPERVISORA!<br/>");
	}
} catch (PDOException $e) {
...
//-----------------------------------------------------------------------------------------------------------------------------		
try {
	$dlt_sql = connection::open('supradaq')->prepare( "DELETE FROM CGOB_TB_SIAC_MEDICAO_MAIOR " );
...
try {
	$insert_sql = connection::open('supradaq')->prepare( "INSERT INTO CGOB_TB_SIAC_MEDICAO_MAIOR (contrato
			,nume_medicao
	...
	if($exceptiond){
		...
		print("Insert $qtdInsMedicao rows CGOB_TB_MEDICAO_OBRA!<br/>");
	}
...
//-----------------------------------------------------------------------------------------------------------------------------
try {
	$dlt_sql = connection::open('supradaq')->prepare( "DELETE FROM CGOB_TB_SIAC_EMPENHO " );
	$exception = $dlt_sql->execute();
...
try {
	$pl_sql = connection::open('sindnit')->prepare("SELECT SK_CONTRATO,
															NU_CON_FORMATADO,
															DT_CARGA,
															SK_TEMPO,
															DT_EMISSAO_EMPENHO,
															...
															VLR_EMPENHO_AJUSTES,
															VLR_EMPENHO_SALDO,
															VLR_EMPENHO_CONSUMIDO FROM Dados_Empenho");
	$exception = $pl_sql->execute();

	$empenho_sindnit = $pl_sql->fetchAll(PDO::FETCH_ASSOC);

	if($exception){
		$count = $pl_sql->rowCount();
		print(" Empenho $count rows extraida Simdnit -> Sucess!<br/>");
	}

} catch (PDOException $e) {
	print "Ocorreu um erro ao tentar executar esta ação -> Select Empenho.<br/>". $e->getMessage();
	die;
}

try {
	$insert_sql = connection::open('supradaq')->prepare( "INSERT INTO CGOB_TB_SIAC_EMPENHO (SK_CONTRATO,
															NU_CON_FORMATADO,
															DT_CARGA,
															SK_TEMPO,
															DT_EMISSAO_EMPENHO,
															...
															VLR_EMPENHO_AJUSTES,
															VLR_EMPENHO_SALDO,
															VLR_EMPENHO_CONSUMIDO
		) VALUES (:SK_CONTRATO, :NU_CON_FORMATADO, :DT_CARGA, :SK_TEMPO, :DT_EMISSAO_EMPENHO, :DT_CARGA_TMP, :SK_SIAFI_NE
		, :NOTA_EMPENHO, :VLR_EMPENHO_INICIAL, :VLR_EMPENHO_AJUSTES, :VLR_EMPENHO_SALDO, :VLR_EMPENHO_CONSUMIDO)" );
	$qtdInsEmpenho = 0;
	foreach ($empenho_sindnit as $value):
		$insert_sql->bindValue(':SK_CONTRATO',$value["SK_CONTRATO"]);
		$insert_sql->bindValue(':NU_CON_FORMATADO',$value["NU_CON_FORMATADO"]);
		$insert_sql->bindValue(':DT_CARGA',$value["DT_CARGA"]);
		$insert_sql->bindValue(':SK_TEMPO',$value["SK_TEMPO"]);
		$insert_sql->bindValue(':DT_EMISSAO_EMPENHO',$value["DT_EMISSAO_EMPENHO"]);
		$insert_sql->bindValue(':DT_CARGA_TMP',$value["DT_CARGA_TMP"]);
		$insert_sql->bindValue(':SK_SIAFI_NE',$value["SK_SIAFI_NE"]);
		$insert_sql->bindValue(':NOTA_EMPENHO',$value["NOTA_EMPENHO"]);
		$insert_sql->bindValue(':VLR_EMPENHO_INICIAL',$value["VLR_EMPENHO_INICIAL"]);
		$insert_sql->bindValue(':VLR_EMPENHO_AJUSTES',$value["VLR_EMPENHO_AJUSTES"]);
		$insert_sql->bindValue(':VLR_EMPENHO_SALDO',$value["VLR_EMPENHO_SALDO"]);
		$insert_sql->bindValue(':VLR_EMPENHO_CONSUMIDO',$value["VLR_EMPENHO_CONSUMIDO"]);
		$exceptiond = $insert_sql->execute();
		$qtdInsEmpenho++;
	endforeach;

	if($exceptiond){
		print("Insert $qtdInsEmpenho rows CGOB_TB_SIAC_EMPENHO!<br/>");
	}

} catch (PDOException $e) {
	print "Ocorreu um erro ao tentar executar esta ação -> Inserir Empenho.<br/>". $e->getMessage();
	die;
}
