...
function editar(infraestrutura){
	relatorio = confereRelatorio();
	if (relatorio == 1) {
		mensagemRelatorioFechado();
	} else {
		...
		//----------------------------------------------------------------------------------------------------------------------------------------
		bootbox.confirm("Confirmar operação [EDITAR CONTROLE PLUVIOMÊTRICO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: 'POST',
					url: base_url + 'index_cgop.php/ControlePluvEditar',
					data: {
...
function recuperaControlePluv() {
	...
	$('#tableControlePluviometrico').dataTable({
		...
		"aoColumns": [
			...
			{data: 'dias', "sClass": "text-center", "width": "60%"},
