...
$().ready(function () {
	...
	$("#insereLicencaAmbiental").click(function () {
		...
		//----------------------------------------------------------------------
		bootbox.confirm("Confirmar operação [INSERIR LICENÇA]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
...
function recuperaLicencaAmbiental() {
	...
	$("#tableLicencasAmbientais").dataTable({
		...
		"aoColumns": [
			{data: "NOME_INFRAESTRUTURA", "sClass": "text-center", "width": "10%"},
...
function populaInfraEstruturaEditar() {
	$.ajax({
		...
		dataType: 'json',
		success: function (data) {
			var servico = $('select[id=nome_infraestruturaEditar]');
			servico.html('');
			servico.append('<option value="" selected >Selecione</option>');
			for (i = 0; i < data.id_tipo_licenca.length; i++) {
				servico.append('<option value="' + data.id_tipo_licenca[i] + '">' + data.desc_tipo_licenca[i] + '</option>');
