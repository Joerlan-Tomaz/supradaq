...
$().ready(function () {
	...
	$("#insereAtividade").click(function () {
		...
		if (fileUpload == "") {
			...
			$.notify("Insira o anexo de Diario da Operação", 'warning');
			return false;
		}
		document.getElementById('fileUpload').style.borderColor = '#d2d6de';
		bootbox.confirm("Confirmar operação [INSERIR DOCUMENTO]?", function (result) {
			if (result === true) {
				...
				$.ajax({
					url: base_url + 'index_cgop.php/AtividadeExecutoraInsereDaq',
					type: "POST",
					data: form,
					dataType: 'json',
					contentType: false,
...
function recuperaAtividadeExecutora() {
	...
	$('#tableAtividadesExecutora').dataTable({
		...
		"sAjaxSource": base_url + "index_cgop.php/AtividadeExecutoraRecuperaDaq",
...
function excluirArquivo(id_arquivo) {
	bootbox.confirm("Confirmar operação [EXCLUIR ARQUIVO]?", function (result) {
		if (result === true) {
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgop.php/AtividadeExecutoraExcluirDaq',
...
function baixarModelo(nome_arquivo) {
	$.ajax({
		url: base_url + 'index_cgop.php/baixarModelo',
		type: 'POST',
