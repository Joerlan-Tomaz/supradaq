    ...
    $("#insereRelatorioLevantamentoHidrografico").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR RELATÓRIO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/RelatorioLevantamentoHidrograficoInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [NÃO HOUVERAM ATIVIDADES ESTE MÊS]?", function (result) {
				if (result === true) {
					$.ajax({
						type: 'POST',
						url: base_url + 'index_cgop.php/RelatorioLevantamentoHidrograficoNaoAtividadeDaq',
						data: {
							periodo: termo
						},
						dataType: 'json',
...
function recuperaRelatorioLevantamentoHidrografico() {
    ...
    $('#tableRelatorioLevantamentoHidrografico').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/RelatorioLevantamentoHidrograficoRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgop.php/RelatorioLevantamentoHidrograficoExcluirDaq",
...
function confereNaoAtividade() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "index_cgop.php/RelatorioLevantamentoHidrograficoConfereAtvDaq",
