    ...
    $("#insereComponenteAmbiental").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR COMPONENTE AMBIENTAL]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/MonitoramentoInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [NÃO HOUVE ATIVIDADES ESTE MÊS]?", function (result) {
				if (result === true) {
					$.ajax({
						type: 'POST',
						url: base_url + 'index_cgop.php/MonitoramentoNaoAtividadeDaq',
						data: {
							periodo: termo
						},
						dataType: 'json',
...
function recuperaComponenteAmbiental() {
    ...
    $('#tableComponenteAmbiental').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/MonitoramentoRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + 'index_cgop.php/MonitoramentoExcluirDaq',
...
function excluirArq(nome_arquivo) {
    $.ajax({
        url: base_url + 'index_cgop.php/excluirArq',
        type: 'POST',
        data: {nome_arquivo: nome_arquivo},
        success: function (data) {
...
function confereNaoAtividade() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "index_cgop.php/MonitoramentoConfereAtvDaq",
