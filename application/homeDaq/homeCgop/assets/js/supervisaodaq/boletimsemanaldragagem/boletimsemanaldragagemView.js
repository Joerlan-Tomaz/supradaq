    ...
    $("#insereBoletimSemanalDragagem").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR BOLETIM]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/BoletimSemanalDragagemInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [NÃO HOUVERAM ATIVIDADES ESTE MÊS]?", function (result) {
				if (result === true) {
					$.ajax({
						type: 'POST',
						url: base_url + 'index_cgop.php/BoletimSemanalDragagemNaoAtividadeDaq',
						data: {
							periodo: termo
						},
						dataType: 'json',
...
function recuperaBoletimSemanalDragagem() {
    ...
    $('#tableBoletimSemanalDragagem').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/BoletimSemanalDragagemRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgop.php/BoletimSemanalDragagemExcluirDaq",
...
function confereNaoAtividade() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "index_cgop.php/BoletimSemanalDragagemConfereAtvDaq",
