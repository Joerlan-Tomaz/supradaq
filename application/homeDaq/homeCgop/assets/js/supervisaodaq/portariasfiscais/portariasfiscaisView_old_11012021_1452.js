...
//------------------------------------------------------------------------------
function editarPortariasFiscais(id_portaria_fiscal, id_arquivo) {
	relatorio = confereRelatorio();
	if (relatorio == 1) {
		mensagemRelatorioFechado();
	} else {
		bootbox.confirm("Confirmar operação [EDITAR PORTARIAS DE FISCAIS]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
					url: base_url + "index_cgop.php/PortariasFiscaisBuscarDadosDaq",
					data: {id_portaria_fiscal: id_portaria_fiscal, id_arquivo: id_arquivo},
					dataType: "json",
					success: function (data) {
						$("#nova_portariafiscal").hide();
						...
						$("#search").show();
						populaUF();
						$("#id_portaria_fiscal").val(data.id_portaria_fiscal);
						$("#unidade_local").val(data.unidade_local);
						$("#n_portaria").val(data.n_portaria);
						$("#data_portaria").val(data.data_portaria);
						$("#contrato_fiscalizado").val(data.contrato_fiscalizado);
						$("#titularidade").val(data.titularidade);
						$("#nome").val(data.nome);
