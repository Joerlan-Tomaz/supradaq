    ...
    $("#insereRelatorioMonitoramentoAmbiental").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR RELATÓRIO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/RelatorioMonitoramentoAmbientalInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [NÃO HOUVERAM ATIVIDADES ESTE MÊS]?", function (result) {
				if (result === true) {
					$.ajax({
						type: 'POST',
						url: base_url + 'index_cgop.php/RelatorioMonitoramentoAmbientalNaoAtividadeDaq',
						data: {
							periodo: termo
						},
						dataType: 'json',
...
function recuperaRelatorioMonitoramentoAmbiental() {
    ...
    $('#tableRelatorioMonitoramentoAmbiental').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/RelatorioMonitoramentoAmbientalRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgop.php/RelatorioMonitoramentoAmbientalExcluirDaq",
...
function confereNaoAtividade() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "index_cgop.php/RelatorioMonitoramentoAmbientalConfereAtvDaq",
