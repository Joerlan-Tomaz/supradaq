    ...
    $("#insereRelatorioMensalDragagem").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR RELATÓRIO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/RelatorioMensalDragagemInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [NÃO HOUVERAM ATIVIDADES ESTE MÊS]?", function (result) {
				if (result === true) {
					$.ajax({
						type: 'POST',
						url: base_url + 'index_cgop.php/RelatorioMensalDragagemNaoAtividadeDaq',
						data: {
							periodo: termo
						},
						dataType: 'json',
...
function recuperaRelatorioMensalDragagem() {
    ...
    $('#tableRelatorioMensalDragagem').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/RelatorioMensalDragagemRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgop.php/RelatorioMensalDragagemExcluirDaq",
...
function confereNaoAtividade() {
	...
	$.ajax({
		type: "POST",
		url: base_url + "index_cgop.php/RelatorioMensalDragagemConfereAtvDaq",
