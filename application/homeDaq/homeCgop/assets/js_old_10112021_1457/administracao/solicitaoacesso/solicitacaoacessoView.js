...
$(document).ready(function (){
    $('#table_solicitacao_acesso').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgob.php/solicitacaoacesso_table_solicitacao_acesso",
        "aoColumns": [
...
function insereUsuario(id, nome, email, empresa, cpf, telefone, coordenacao, id_uf) {     
    bootbox.confirm("Confirmar operação [ACEITAR SOLICITAÇÃO DE ACESSO]?", function (result) {
        if (result === true) {
            $.ajax({
                type: 'POST',
                url: base_url + "index_cgob.php/solicitacaoacesso_insereUsuario",
                data: {
                    id: id,
                    nome: nome,
                    email: email,
...
function negaSolicitacao(id) {
    bootbox.confirm("Confirmar operação [NEGAR SOLICITAÇÃO DE ACESSO]?", function (result) {
        if (result === true) {
            $.ajax({
                type: 'POST',
                url: base_url + "index_cgob.php/solicitacaoacesso_negaSolicitacao",
                data: {id: id},
                dataType: 'json',
                success: function (data) {
