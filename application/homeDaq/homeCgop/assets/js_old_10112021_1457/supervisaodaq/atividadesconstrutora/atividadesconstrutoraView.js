    ...
    $("#insereServicoExecutado").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR ATIVIDADE CONSTRUTORA]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/AtividadesConstInsereDaq',
                    data: serializedData,
                    dataType: 'json',
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR SERVIÇO EXECUTADO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/AtividadeConstExcluirDaq',
                    data: {
                        id_resumo: id_resumo
                    },
                    dataType: 'json',
...
function recuperaServicosExecutados() {
    ...
    $('#tableServicoExecutado').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/AtividadesConstRecuperaDaq",
...
function editarAtividadeConstrutora(id_resumo) { 
    ...
    if (relatorio == 1) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgop.php/AtividadeConstEditarDaq',
...
function btnRecuperaUltimo() {
    ...
    bootbox.confirm("Confirmar operação [Recuperar dados do periodo referência anterior]?", function (result) {
      if (result === true) {
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgop.php/RecuperaUltimoAtv?periodo='+termo,
            dataType: 'json',
            success: function (data) {               
               if(data.resumo != null){
