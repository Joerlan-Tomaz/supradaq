...
//-------------------------------------------------------------------------------------------------------
    public function adicionaDadosInicio($dados) {
        ...
        SELECT
            ...
            (SELECT SUM(val_final) FROM CGOB_TB_AVANCO_FISICO WHERE publicar like '%S%'AND id_contrato_obra = '" . $dados["idContrato"] . "' AND servico = af.id_servico AND obra = af.id_obra) as atacado

        FROM CGOB_TB_CRONOGRAMA_FISICO AS af


        WHERE (af.publicar = 'S' or af.publicar is NULL) AND af.unidade_medida not in ('SN') AND af.id_contrato_obra = '" . $dados["idContrato"] . "' AND af.versao = (select MAX (versao) FROM CGOB_TB_CRONOGRAMA_FISICO where id_contrato_obra = '" . $dados["idContrato"] . "' AND publicar = 'S' AND publicar_versao ='S')
