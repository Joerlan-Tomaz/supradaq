...
//-------------------------------------------------------------------------------------------------------
 public function adicionaDadosInicio($dados) {
        ...
        SELECT
            ...
            (SELECT SUM(val_final) FROM CGOB_TB_AVANCO_FISICO WHERE publicar like '%S%'AND id_contrato_obra = '" . $dados["idContrato"] . "' AND id_servico = af.id_servico AND id_obra = af.id_obra AND versao ='" . $dados["versao"] . "') as atacado
