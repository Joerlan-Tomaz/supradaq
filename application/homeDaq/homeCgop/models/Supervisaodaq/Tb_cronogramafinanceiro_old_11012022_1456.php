...
//----------------------------------------------------------------------------------------------------------------------------	
	public function RecuperaCronogramaAgrupado_naopublicado($dados)
	{
		$SQL = "
		SELECT
		convert(varchar, SUM(convert(money, c.valor_inicial) + convert(money, c.Valor_Total_de_Reajuste)), 1) AS valor_inicial,
...
//------------------------------------------------------------------------------------------------------------------------
	...
	{
		$SQL = "SELECT concat(cronograma.obra,' - ',cronograma.desc_obra, ' - ', cronograma.servico, ' - ' , cronograma.desc_tipo) as  servico,
