...
//-----------------------------------------------------------------------------------
    ...
    public function JustificativaDeEmpreendimentoTodos($dados) {
        $SQL = "
        SELECT 
           resumo,
		   (select desc_nome from TB_USUARIO where id_usuario=r.id_usuario) nome,
		   concat( CONVERT(CHAR(10),r.ultima_alteracao , 103),' ', CONVERT(CHAR(8),r.ultima_alteracao , 114)) AS ultima_alteracao
           FROM CGOB_TB_RESUMO r
        WHERE id_contrato_obra = ". $dados["id_contrato_obra"] ." AND id_roteiro in ('14') AND publicar = 'S' 
            ";
    ...
    public function mapasituacao($dados) {
        $SQL = "
          SELECT
            nome_arquivo as mapa_situacao
            FROM CGOB_TB_ARQUIVO 
            WHERE id_contrato_obra = " . $dados["id_contrato_obra"] . " AND roteiro in ('15') AND publicar = 'S'
            ";
...
public function Mobilizacao_SICRO_Supervisora_Pessoal($dados) {
    $SQL ="
        ...
        (SELECT nome_infraestrutura FROM CGOB_TB_SICRO_QUANTIDADE_SR WHERE periodo_referencia = '" . $dados["periodo"] . "' and id_sicro_supervisora = s.id_sicro_supervisora)  nome_infraestrutura
        ...
        WHERE (sr.publicar like '%S%' or sr.publicar is NULL) AND s.item like '%PESSOAL%' 
...
public function AtividadeSupervisora($dados) {
    ...
    FROM CGOB_TB_RESUMO WHERE 
    publicar = 'S' AND id_contrato_obra = ". $dados["id_contrato_obra"] ." AND periodo_referencia = '". $dados["periodo"] ."' AND id_roteiro in ('9') 
    ";
    $query = $this->db->query($SQL);
    return $query->result_array();
}
...
public function Mobilizacao_SICRO_Construtora_Pessoal($dados) {
    $SQL ="
        ...
        (SELECT nome_infraestrutura FROM CGOB_TB_SICRO_QUANTIDADE WHERE periodo_referencia = '" . $dados["periodo"] . "' and id_sicro_construcao = s.id_sicro_construcao)  nome_infraestrutura
...
//-------------------------------------------------------------------------------------------------------------------
        ...
        concat(CONVERT(CHAR(10), rnc.data_alteracao , 103),' ', CONVERT(CHAR(8), rnc.data_alteracao , 114)) AS ultima_alteracao
        FROM CGOB_TB_RNC as rnc 
        LEFT JOIN CGOB_TB_RNC_ITEM_NATUREZA as nat ON nat.id_natureza = rnc.id_natureza 
        LEFT JOIN CGOB_TB_RNC_ITEM_GRAVIDADE as grau ON grau.id_gravidade = rnc.id_gravidade 
        FULL OUTER JOIN CGOB_TB_RNC_PROVIDENCIA as prov on prov.id_rnc = rnc.id_rnc
        WHERE rnc.publicar = 'S' AND rnc.id_contrato_obra =" .$dados['idContrato']. "
        AND rnc.periodo_referencia <='" . $dados["periodo"] . "'     
...
//-----------------------------------------------------------------------------------------------------------------------
    ...
    public function recuperaAnexosGaratias($dados) {
        ...
        rnc.km,
        rnc.descricao, 
        rnc.nome_eixo as desc_tipo_eixo,
        convert(varchar(10),rnc.data_registro,103) AS data_registro, 
        CASE WHEN  (prov.situacao_providencia IS NULL or prov.situacao_providencia like '' OR prov.situacao_providencia = 'Aberto')  THEN 'Aberto' 
        ELSE 'Fechado' 
        END as status, 
        ...
        FROM CGOB_TB_RNC as rnc 
        LEFT JOIN CGOB_TB_OBRA as nat ON nat.id_tipo_obra = rnc.id_natureza 
        LEFT JOIN CGOB_TB_RNC_ITEM_GRAVIDADE as grau ON grau.id_gravidade = rnc.id_gravidade 
        FULL OUTER JOIN CGOB_TB_RNC_PROVIDENCIA as prov on prov.id_rnc = rnc.id_rnc
