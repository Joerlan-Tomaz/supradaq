...
class LicencasAmbientais extends CI_Controller
{
	...
	{
		$DadosServico = $this->Tb_licencas_ambientais->populaNomeInfra();
		$n = 0;
		foreach ($DadosServico as $lista) {
			$dados['id_tipo_licenca'][$n] = $lista->nome_eixo;
			$dados['desc_tipo_licenca'][$n] = str_replace("_", " ", $lista->nome_eixo);
			$n++;
		}
		echo(json_encode($dados));
	}
...
//------------------------------------------------------------------------------------------------------------------------
	...
	public function RecuperaLicencaEditar()
	{
		...
		foreach ($DadosServico as $lista) {
			...
			$dados['nome_infraestrutura'] = $lista->nome_infraestrutura;
