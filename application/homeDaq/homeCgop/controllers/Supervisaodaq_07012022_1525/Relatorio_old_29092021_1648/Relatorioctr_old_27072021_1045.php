...
//----------------------------------------------------------------------------------------------------------
	...
	public function validaperfil(){
		...
		//-----------------------------------------------------------------
		/*if (!empty($dados["id_perfil_analise"]) || $dados["id_perfil_analise"] != 1) {
			$retorno["mensagem"] = "[Não possui perfil para conclusão e envio do relatorio. Solicite ao responsável.]";
			//$retorno["notify"] = "warning";
			die (json_encode($retorno));
		}
		$retorno = true;*/
                if(($dados["id_perfil_analise"] == 1) or $dados["id_perfil_analise"] == 9){
                   $retorno = true; 
                }else{
                   $retorno["mensagem"] = "[Não possui perfil para conclusão e envio do relatorio. Solicite ao responsável.]"; 
                   die (json_encode($retorno));
	...
	public function returnelaboracaodaq()
	{
		...
		if (!empty($resultadoelaboracao)) {
			foreach ($resultadoelaboracao as $imprimir) {
				...
				$a = array("id_aceite" => "aprovado", "perfil" => 2);
				$b = array("id_aceite" => "aprovado", "perfil" => 3);
                                $c = array("id_aceite" => "aprovado", "perfil" => 9);
                                

				$d = array_diff($a, $imprimir);
				$e = array_diff($b, $imprimir);
                                $f = array_diff($c, $imprimir);
		...
		if (!empty($AndamentoanaliseDaq) and ($var1 == 0 || $var2 == 0 || $var3 == 0)) {
			foreach ($AndamentoanaliseDaq as $andamento) {

				$aceite = $andamento->id_aceite;
				$roteiro = $andamento->roteiro_analise;
				$perfil = $andamento->perfil;

				//---------------------------------------------------------------
				if ($aceite == "aprovado" && ($perfil == 2 or $perfil == 9)) {

					$dados['data'] = "aguardando_analise_fiscal";
				}
				if ($aceite == "retificado" && ($perfil == 2 or $perfil == 9)) {

					$dados['data'] = "liberar_relatorio";
				}
				if ($aceite == "reprovado" && ($perfil == 2 or $perfil == 9)) {

					$dados['data'] = "ReprovadoAnalista";
				}
				//-----------------------------------------------------------------
				if ($aceite == "aprovado" && ($perfil == 3 or $perfil == 9)) {

					$dados['data'] = "aguardando_analise_analista";
				}
				if ($aceite == "retificado" && ($perfil == 3 or $perfil == 9)) {

					$dados['data'] = "liberar_relatorio";
				}
				if ($aceite == "reprovado" && ($perfil == 3 or $perfil == 9)) {

					$dados['data'] = "ReprovadoFiscal";
				}
				//-----------------------------------------------------------------
				if ($aceite == "aguardando análise" && ($perfil == 1 or $perfil == 9)) {
...
//-----------------------------------------------------------------------------------------------------------
	public function returnrecibodaq()
	{
		...
		$this->load->view('homeDaq/supervisaodaq/relatorio/ReciboSupervisao', $DadosRecibo);
	}
...
//--------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------#
public function Recupera_elaboracao() {
        $dados["idContrato"] = $this->session->idContrato;
        $dados["periodo"] = $this->input->post_get("periodo");
        $DadosElaboracao = $this->Tb_relatorio->Recupera_elaboracao($dados);

        foreach ($DadosElaboracao as $lista) {
            $dados ['qtd_fechado'] = $lista->qtd_fechado;
        }
        echo (json_encode($dados));
    }
#-------------------------------------------------------------------------------#
