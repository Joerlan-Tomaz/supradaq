...
class DiarioObra extends CI_Controller {
    ...
    function __construct() {
     ...
     public function insereDiarioObra() {
    ...
    public function recuperaDiarioObra() {
        $dados["idContrato"] = $this->session->idContrato;
        $dados["periodo"] = $this->input->post_get("periodo");
        $dados["roteiro"] = "31";
        ...
        if (!empty($Dados)) {
            foreach ($Dados as $i => $lista) {
				...
				$path_imagem = base_url('index_cgob.php/70bc1de8a077e52493d9c41ffaa3c051ARQ?arq=' . $lista->nome_arquivo);
                $arquivo = "<a download href='" . $path_imagem . "' target='_blank'>" . $nomeArquivo . "<a>";
