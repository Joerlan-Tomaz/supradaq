	...
	public function recuperaContrato()
	{
		...
		if (!empty($DadosAnaliseRelatorio)) {
			foreach ($DadosAnaliseRelatorio as $lista) {
				...
				//------------------------------------------------------------------------------
				if (($relatorio == "") or ($aceite == "aguardando análise" && $roteiro_analise == "fechar_relatorio" && $perfil == 5) or ($aceite == "aguardando análise" && $roteiro_analise == "fechar_relatorio" && $perfil == 2)) {
					$status_versao = "Aguardando Análise - [" . $versao . "]";
...
//		$ImpressaoRelatorioDaq = $this->Tb_relatorio->DadosImpressaoRelatorioDaq($dados);
		$validacaoTela = $this->Tb_telas_validacao->busca_tela_analise($dados);

		//------------------------------------------------------------------------------
		$num = 1;
		if (!empty($validacaoTela)) {
			foreach ($validacaoTela as $imprimir) {

				$aceite = '';
				$dados["id_modulo"] = $imprimir->id_tela_formulario;
				...
				if ($aceite == "retificado") {
					$acao = " <a data-toggle='tooltip' title='Status Análise' data-placement='top' style='font-size:20px'>
						...
						<a type = 'button' data-toggle='tooltip' title='Editar' data-placement='top' style='font-size:20px' href='javascript:void(0);' onclick=\"EditarAnalise({$imprimir->id_tela_formulario},'{$imprimir->nome_tela}',{$id_contrato})\"> <i class='fas fa-pen-square' style='color:#FFC107'></i></a>
					...
					$acao = " <a data-toggle='tooltip' title='Status Análise' data-placement='top' style='font-size:20px'>
                                 <i class=' fas fa-thumbs-up' style='color:green'></i></a>
						<a type = 'button' data-toggle='tooltip' title='Editar' data-placement='top' style='font-size:20px' href='javascript:void(0);' onclick=\"EditarAnalise({$imprimir->id_tela_formulario},'{$imprimir->nome_tela}',{$id_contrato})\"> <i class='fas fa-pen-square' style='color:green'></i></a>
				...
				$dados['data'][] = array(
					"cont" => $num++,
					"modulo" => $imprimir->nome_tela,
					"nome" => $imprimir->DESC_NOME,
					"data" => $imprimir->data_ultima_alteracao,
					"acao" => $acao
...
//------------------------------------------------------------------------------
	public function insere_editar_aceite()
	{
		...
		//------------------------------------------------------------------------------
		$dados['id_contrato_obra'] = $dados["id_contrato"];
		$dados['id_tela_formulario'] = $dados["id_modulo"];
		$this->Tb_telas_validacao->insereAnalise($dados);
...
//---------------------------------------------------------------------------------------------------------------------------------
	public function Relatorioanalise()
	{
		...
		if (!empty($resumorpfo)) {
			...
			$DadosR['resumo_rpfo'] = '<div class="alert alert-danger" role="alert">[3.10 REVISÕES DE PROJETO EM FASE DE OPERAÇÔES - RPFO] não cadastrado!</div>';
		}
