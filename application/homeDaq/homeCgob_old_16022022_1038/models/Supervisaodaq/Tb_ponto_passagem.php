...
class Tb_ponto_passagem extends CI_Model {
...
//----------------------------------------------------------------------------------
    ...
    public function detalhesPontoPassagem($dados) {
        $SQL = "            
            SELECT 
                nome
                ,estaca
                ...
                ,id_arquivo
                ,tipo
            FROM CGOB_TB_PONTO_PASSAGEM AS geo
    ...
    public function recuperaPontoPassagem($dados) {
        $SQL = "            
            SELECT  
                id_arquivo
                ,nome_arquivo
                ...
                ,nomeOriginalArquivo
                ,(SELECT count(*) FROM CGOB_TB_PONTO_PASSAGEM WHERE id_contrato_obra = " . $dados["id_contrato"] . " AND (publicar = 'S' OR publicar IS NULL) AND id_arquivo = a.id_arquivo) AS total
            FROM CGOB_TB_ARQUIVO as a

            WHERE id_contrato_obra = " . $dados["id_contrato"] . " AND roteiro in ('20','PONTOPASSAGEM') AND (publicar = 'S' OR publicar IS NULL) 
            ";
    ...
    public function excluirPontoPassagem($dados) {
        ...
        $this->db->update("CGOB_TB_PONTO_PASSAGEM");
    ...
    public function inserirdados($dados) {
        ...
        set_time_limit(300);
        $this->db->insert_batch("CGOB_TB_PONTO_PASSAGEM", $dados);
        return true;
    }
    #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
