	...
	$("#insereJustificativa").click(function () {
		...
		bootbox.confirm("Confirmar operação [INSERIR JUSTIFICATIVA DE EMPREENDIMENTO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
					url: base_url + "index_cgob.php/JustificativaInsereDaq",
					data: serializedData,
					dataType: "json",
...
function recuperaJustificativa() {
	...
	$("#tableJustificativa").dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/JustificativaRecuperaDaq?periodo=" + termo,
	...
	if (relatorio == 1) {
		...
		bootbox.confirm("Confirmar operação [EXCLUIR JUSTIFICATIVA DE EMPREENDIMENTO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
					url: base_url + "index_cgob.php/JustificativaExcluirDaq",
	...
	if (relatorio == 1) {
		...
		bootbox.confirm("Confirmar operação [EXCLUIR JUSTIFICATIVA DE EMPREENDIMENTO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
					url: base_url + "index_cgob.php/JustificativaEditarDaq",
					data: {id_resumo: id_resumo},
					dataType: "json",
					success: function (data) {
						$("#pesquisaJustificativaEmpreendimento").hide();
