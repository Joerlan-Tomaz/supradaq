		...
		if (relatorio == 1) {
			...
			bootbox.confirm("Confirmar operação [INSERIR DIAGRAMA DE PONTO DE PASSAGEM OCORRÊNCIA DE PROJETO]?", function (result) {
				if (result === true) {
					$.ajax({
						type: "POST",
						url: base_url + "index_cgob.php/DiagramaPPInsereDaq",
						data: form,
						dataType: "json",
						contentType: false,
						processData: false,
...
function recuperaDiagramaPontoPassagem() {
	...
	$("#tableDiagramaPontoPassagem").dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/DiagramaPPRecuperaDaq",
	...
	if (relatorio == 1) {
		...
		bootbox.confirm("Confirmar operação [EXCLUIR MAPA DE SITUAÇÃO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: "POST",
					url: base_url + "index_cgob.php/DiagramaPPExcluirDaq",
...
function excluirDiagrama(nome_arquivo) {
	$.ajax({
		url: base_url + 'index_cgob.php/excluirDiagrama',
		type: 'POST',
		data: {nome_arquivo: nome_arquivo},
		success: function (data) {
