...
//        form.append('arquivo', $('#fileUpload')[0].files[0]);
        ...
        bootbox.confirm("Confirmar operação [INSERIR CONCLUSÃO GERAL]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgob.php/ConclusaoInsereDaq',
                    data: form,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
...
function recuperaConclusaoGeral() {
    ...
    $('#tableConclusaoGeral').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgob.php/ConclusaoRetornaDaq",
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR REGISTRO]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgob.php/ConclusaoExcluirDaq",
...
function btnRecuperaUltimo() {
    ...
    bootbox.confirm("Confirmar operação [Recuperar dados do periodo referência anterior]?", function (result) {
      if (result === true) {
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgob.php/RecuperaConGeralDaqUltimo?periodo='+termo,
            dataType: 'json',
            success: function (data) {
               if(data.resumo != null){
