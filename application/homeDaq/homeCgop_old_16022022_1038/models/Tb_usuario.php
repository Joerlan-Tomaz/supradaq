...
class Tb_usuario extends CI_Model
{
	...
	public function RecuperaQtdMensagem()
	{
		$SQL = "        
                SELECT
                    ...
                    FROM 
                    MSG_SAIDA s 
                    where s.flag_lido='N' AND S.id_destinatario = " . $this->session->id_usuario_daq_cgop . "
                    and s.id_roteiro=2 
            ...
            union all
                ...
                SELECT
                    ...
                    FROM 
                    MSG_RESPOSTA r 
                    where r.flag_lido='N' AND r.id_destinatario = " . $this->session->id_usuario_daq_cgop . " 
	...
	public function RecuperaFotoUsuario()
	{
        ...
        FROM 
        TB_USUARIO_PERFIL U 
        WHERE id_usuario = " . $this->session->id_usuario_daq_cgop . "
	...
	public function historicoLogin()
	{
		...
		$this->db->set("id_usuario", $this->session->id_usuario_daq_cgop);
	...
	{
		date_default_timezone_set('America/Sao_Paulo');
		$this->db->where("id_usuario", $this->session->id_usuario_daq_cgop);
	...
	public function finalizaSessao($dados)
	{
		$SQL = "
            UPDATE TB_HISTORICOLOGIN
                ...
                ONLINE = 'N' 
                WHERE ID_HISTORICOLOGIN = (SELECT MAX(ID_HISTORICOLOGIN) FROM TB_HISTORICOLOGIN WHERE ID_USUARIO = " . $this->session->id_usuario_daq_cgop . ")
                ";
		$query = $this->db->query($SQL);
	...
	public function RecuperaLinhaTempo()
	{
        ...
        WHERE 1=1 AND nome = (select desc_nome from tb_usuario where id_usuario = " . $this->session->id_usuario_daq_cgop . ")
	...
	public function RecuperaMensagemPerfil()
	{
        ...
        WHERE MS.id_destinatario = " . $this->session->id_usuario . " or U_REM.id_usuario = " . $this->session->id_usuario_daq_cgop . "
        AND id_roteiro = 2
	...
	public function RecuperaRemetente()
	{
		$SQL = "
            SELECT id_usuario
                ,desc_nome          
            FROM TB_USUARIO
            where 1 = 1 AND id_usuario = " . $this->session->id_usuario_daq_cgop;
		$query = $this->db->query($SQL);
		return $query->result();
	}
	...
	public function RecuperaDestinatario()
	{
        ...
        AND id_usuario <> " . $this->session->id_usuario_daq_cgop . "
	...
	{
		$SQL = "SELECT * FROM TB_USUARIO_ONLINE where id_usuario = " . $this->session->id_usuario_daq_cgop . "";
		$query = $this->db->query($SQL);
		return $query->result();
	}
	...
	public function recuperaUsuarioSessao()
	{
            ...
            WHERE o.id_usuario = " . $this->session->id_usuario_daq_cgop;
...
public function RecuperaDadosGestaoSupra($dados){
		...
		$SQL = "
            SELECT 
                ...
                u.email,u.cpf,u.telefone
            FROM tb_usuario u 
            WHERE id_usuario_daq_cgop = {$_REQUEST['id_usuario_PerfilPermissao']}";
           // echo('<pre>');
           // die($SQL);
		$query = $this->db->query($SQL);
		return $query->result();
...
public function insereUsuarioAcessoSupra($dados) {
        ...
        $this->db->set("id_usuario_daq_cgop", $dados["id_usuario"]);
        $this->db->insert("TB_USUARIO");
         $this->db->trans_complete();
        if ($this->db->trans_status() === true) {
            $this->db->trans_commit();
