...
//---------------------------------------------------------------------------------------------
		...
		WHERE (cf.publicar = 'S') 
		  AND cf.publicar_versao = 'N' 
		  AND cf.nome_eixo ='" . $dados["nome_eixo"] . "'  
		...
		";
        if (!empty($dados["tipo"])) {
            $SQL .= "AND cf.tipo_obra = '" . $dados["tipo"] . "' ";
        }
...
//---------------------------------------------------------------------------------------------
	public function CronogramaPublicado($dados){
		...
		FULL OUTER JOIN CGOB_TB_CONFIG_GEORREFERENCIAMENTO AS g ON v.nome_eixo = g.nome_eixo
		WHERE v.publicar_cronograma = 'S' 
		  AND v.id_contrato_obra = '" . $dados["id_contrato_obra"] . "'
		group by (g.nome_eixo)
		";
		$query = $this->db->query($SQL);
		return $query->result();
	}
...
//---------------------------------------------------------------------------------------------------
	public function CronogramaNaoPublicado($dados) {
		$SQL = "
		SELECT 
		distinct(g.nome_eixo) as nome_eixo,
		MAX (g.km) as maior,
		MAX(g.km)-MIN(g.km) as extensao,
		v.id_usuario, 
		CASE 
		...
		(SELECT count(1)publicar_versao FROM CGOB_TB_CRONOGRAMA_FISICO as f 
			WHERE f.nome_eixo = g.nome_eixo and f.publicar_versao = 'N' 
			  AND f.publicar = 'S' 
			  AND f.id_contrato_obra = '" . $dados["id_contrato_obra"] . "' 
			  AND f.versao = v.versao) publicar
		FROM CGOB_TB_CONFIG_GEORREFERENCIAMENTO AS g
		FULL OUTER JOIN CGOB_TB_CRONOGRAMA_FISICO_VERSAO AS v  ON v.publicar_cronograma = 'N' 
			AND v.id_contrato_obra = '" . $dados["id_contrato_obra"] . "' AND g.nome_eixo = v.nome_eixo
			WHERE (g.publicar = 'S' OR g.publicar IS NULL) 
			AND g.id_contrato_obra = '" . $dados["id_contrato_obra"] . "'
		";

		$SQL .= " group by g.nome_eixo, g.eixo, v.id_usuario, v.versao, v.id_cronograma,v.publicar_cronograma ";
	...
	public function insereVersao($dados){
		...
		$this->db->set("nome_eixo", $dados['nome_eixo']);
...
//-------------------------------------------------------------------------------------------------
	public function ServicosInseridosAquaviario($dados){
		...
		WHERE cf.publicar = 'S'AND cf.publicar_versao = 'N' AND cf.nome_eixo = '" . $dados["nome_eixo"] . "'
		";
		...
		if (!empty($dados["servico"])) {
			$SQL .= "AND cf.id_servico = '" . $dados["servico"] . "' ";
		}
		if (!empty($dados["tipo"])) {
			$SQL .= "AND cf.tipo_obra = '" . $dados["tipo"] . "' ";
		}
...
//---------------------------------------------------------------------------------------------------------
	public function RecuperaGeorreferenciamento($dados){
		...
		nome_eixo
		FROM CGOB_TB_CRONOGRAMA_FISICO
		WHERE nome_eixo = " .$dados["nome_eixo"]. "
...
//---------------------------------------------------------------------------------------------------------
	public function DetalhadoNaoPublicado($dados){
		...
		if (!empty($dados["nome_eixo"])) {
			$SQL .= " AND cf.nome_eixo = '" . $dados["nome_eixo"] . "' ";
		}
		if (!empty($dados["id_cronograma"])) {
			$SQL .= " AND cf.id_cronograma = '" . $dados["id_cronograma"] . "' ";
		}
...
//----------------------------------------------------------------------------------------------------------------------
	public function PublicarCronograma_naopublicado($dados){
		...
		$this->db->where("nome_eixo", $dados['nome_eixo']);
	...
	public function acompanhementofisico($dados){
		$SQL = "
				...
				FROM CGOB_TB_CRONOGRAMA_FISICO cf
				 ...
				 INNER JOIN CGOB_TB_OPERACAO_SERVICO AS os ON os.id_servico = cf.id_servico
				 LEFT JOIN CGOB_TB_OPERACAO_TIPO AS ot ON ot.id_tipo = cf.id_tipo_operacao
				WHERE cf.publicar_versao like '%S%'
				  ...
				  and cf.versao = (SELECT MAX(versao)
								   ...
								   where id_contrato_obra = '" . $dados["id_contrato_obra"] . "'
									 ...
									 and publicar like '%S%')
				group by cf.nome_eixo, op.nome_operacao, os.servico, ot.tipo, cf.ano, cf.mes, cf.unidade_medida
				union all
				...
				WHERE af.publicar_versao = 'S'
				  ...
				  AND af.versao = (SELECT MAX(versao) FROM CGOB_TB_AVANCO_FISICO where id_contrato_obra = '" . $dados["id_contrato_obra"] . "')
				group by af.infraestrutura, op.nome_operacao, os.servico, ot.tipo, year(af.periodo_referencia), af.mes, af.unidade_medida, af.periodo_referencia
			)
			select concat(obra, ' - ', desc_servico) as desc_servico,
				   ano,
				   unidade_medida,
				   isnull(SUM(CASE
				   ...
				   isnull(SUM(CASE
								  WHEN mes = 1 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_01,
				   isnull(SUM(CASE
								  WHEN mes = 2 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_02,
				   isnull(SUM(CASE
								  WHEN mes = 3 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_03,
				   isnull(SUM(CASE
								  WHEN mes = 4 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_04,
				   isnull(SUM(CASE
								  WHEN mes = 5 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_05,
				   isnull(SUM(CASE
								  WHEN mes = 6 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_06,
				   isnull(SUM(CASE
								  WHEN mes = 7 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_07,
				   isnull(SUM(CASE
								  WHEN mes = 8 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_08,
				   isnull(SUM(CASE
								  WHEN mes = 9 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_09,
				   isnull(SUM(CASE
								  WHEN mes = 10 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_10,
				   isnull(SUM(CASE
								  WHEN mes = 11 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_11,
				   isnull(SUM(CASE
								  WHEN mes = 12 and identit = 'c'
									  THEN valormedido
								  ELSE 0 END), 0) concluido_12,
				   max(maior) as                  mesbreak
			from previsto
			group by obra, desc_servico, ano, unidade_medida
			order by ano";
				$query = $this->db->query($SQL);
//				echo '<pre>';
//				var_dump($SQL);
...
//-----------------------------------------------------------------------------------------------------------------
	...
	public function excluirItens($dados){
        date_default_timezone_set('America/Sao_Paulo');
        $this->db->where("id_contrato_obra", $dados['id_contrato_obra']);
        $this->db->where("ano", $dados['ano']);
        $this->db->where("nome_eixo", $dados['nome_eixo']);
        $this->db->where("id_operacao", $dados['id_operacao']);
        $this->db->where("id_servico", $dados['id_servico']);
		if (!empty($dados["id_tipo_operacao"])) {
			$this->db->where("id_tipo_operacao", $dados['id_tipo_operacao']);
		}
        $this->db->set("publicar", "N");
        $this->db->set("data_publicar", date("Y-m-d H:i:s"));
        $this->db->set("id_usuario_publicar", $dados['idUsuario']);
        $this->db->update("CGOB_TB_CRONOGRAMA_FISICO");
