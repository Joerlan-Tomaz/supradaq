...
class Tb_usuario extends CI_Model
{
	...
	public function RecuperaQtdMensagem()
	{
		$SQL = "        
                SELECT
                    ...
                    FROM 
                    MSG_SAIDA s 
                    where s.flag_lido='N' AND S.id_destinatario = " . $this->session->id_perfil_daq . "
                    and s.id_roteiro=2 
            ...
            union all
                ...
                SELECT
                    ...
                    FROM 
                    MSG_RESPOSTA r 
                    where r.flag_lido='N' AND r.id_destinatario = " . $this->session->id_perfil_daq . " 
	...
	public function RecuperaFotoUsuario()
	{
        ...
        FROM 
        TB_USUARIO_PERFIL U 
        WHERE id_usuario = " . $this->session->id_perfil_daq . "
	...
	public function historicoLogin()
	{
		...
		$this->db->set("id_usuario", $this->session->id_perfil_daq);
	...
	{
		date_default_timezone_set('America/Sao_Paulo');
		$this->db->where("id_usuario", $this->session->id_perfil_daq);
	...
	public function finalizaSessao($dados)
	{
		$SQL = "
            UPDATE TB_HISTORICOLOGIN
                ...
                ONLINE = 'N' 
                WHERE ID_HISTORICOLOGIN = (SELECT MAX(ID_HISTORICOLOGIN) FROM TB_HISTORICOLOGIN WHERE ID_USUARIO = " . $this->session->id_perfil_daq . ")
                ";
		$query = $this->db->query($SQL);
	...
	public function RecuperaLinhaTempo()
	{
        ...
        WHERE 1=1 AND nome = (select desc_nome from tb_usuario where id_usuario = " . $this->session->id_perfil_daq . ")
	...
	public function RecuperaMensagemPerfil()
	{
        ...
        WHERE MS.id_destinatario = " . $this->session->id_perfil_daq . " or U_REM.id_usuario = " . $this->session->id_perfil_daq . "
        AND id_roteiro = 2
	...
	public function RecuperaRemetente()
	{
		$SQL = "
            SELECT id_usuario
                ,desc_nome          
            FROM TB_USUARIO
            where 1 = 1 AND id_usuario = " . $this->session->id_perfil_daq;
		$query = $this->db->query($SQL);
		return $query->result();
	}
	...
	public function RecuperaDestinatario()
	{
        ...
        AND id_usuario <> " . $this->session->id_perfil_daq . "
	...
	{
		$SQL = "SELECT * FROM TB_USUARIO_ONLINE where id_usuario = " . $this->session->id_perfil_daq . "";
		$query = $this->db->query($SQL);
		return $query->result();
	}
	...
	public function alterarDadosUsuario($dados)
	{
		...
		$this->db->set("TELEFONE", $dados["telefone"]);
		$this->db->set("empresa", $dados["empresa"]);
		$this->db->set("id_usuario_alteracao", $this->session->id_perfil_daq);
	...
	public function alteraPerfilPermissao_old_07072021_2001($dados,$sistema)
	{
		date_default_timezone_set('America/Sao_Paulo');
		$this->db->where("id_usuario", $dados["id_usuario_PerfilPermissao"]);
		$this->db->set("ID_PERFIL", '0');
		if($sistema == 'DAQ'){
	...
	public function RecuperaInfoUsuario()
	{
		$SQL = "
            SELECT USU.id_usuario,
                   USU.DESC_NOME, 
				   USU.EMAIL, 
				  (select desc_perfil from tb_perfil where id_perfil=usu.id_perfil) as desc_perfil, 
				   (select no_empresa from CGOB_TB_CONTRATO_SUPERVISORA where id_contrato_supervisora=usu.id_supervisora) as no_empresa, 
				   USU.empresa,
                   usu.id_perfil
