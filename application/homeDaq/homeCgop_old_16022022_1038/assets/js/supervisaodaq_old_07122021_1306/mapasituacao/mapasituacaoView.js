...
$().ready(function () {
	...
	$("#fileUploadConfigMapa").change(function () {
		...
		if (relatorio == 1) {
			...
			form.append('id_tela_formulario', $('#id_tela_formulario').val());
			//----------------------------------------------------------------------
			bootbox.confirm("Confirmar operação [INSERIR MAPA DE SITUAÇÃO]?", function (result) {
				if (result === true) {
					$.ajax({
						type: "POST",
						url: base_url + "index_cgop.php/MapaSituacaoInsereDaq",
						data: form,
						dataType: "json",
						contentType: false,
						processData: false,
...
function anexoMapas(nome_arquivo) {
	$.ajax({
		url: base_url + 'index_cgop.php/anexoMapas',
		type: 'POST',
...
function recuperaMapaSituacao() {
	...
	$("#tableMapaSituacao").dataTable({
		...
		"sAjaxSource": base_url + "index_cgop.php/MapaSituacaoRecuperaDaq",
...
function excluirMapas(nome_arquivo) {
	$.ajax({
		url: base_url + 'index_cgop.php/excluirMapas',
		type: 'POST',
		data: {nome_arquivo: nome_arquivo},
		success: function (data) {
	...
	if (relatorio == 1) {
		...
		bootbox.confirm("Confirmar operação [MAPA SITUAÇÃO]?", function (result) {
			if (result === true) {
				$.ajax({
					type: 'POST',
					url: base_url + 'index_cgop.php/MapaSituacaoExcluirDaq',
