...
function recuperaAvancoFisicoaquaviario() {
	...
	$('#tableAvancoFisicoConcluidos').dataTable({
		...
		"aoColumns": [
			...
			{data: 'operacao', "sClass": "text-center"},
			{data: 'servico', "sClass": "text-center"},
			{data: 'tipo', "sClass": "text-center"},
			{data: 'versao', "sClass": "text-center"},
			{data: 'executado_em', "sClass": "text-center"},
...
//------------------------------------------------------------------------------------------------------------------------------   
	var medicao = $("#medicao").val();
	var infraestrutura = $("#infraestrutura").val();
	...
	//----------------------------------------------------------------------------------------------//
	var Validar = Cronograma_fisico(infraestrutura, id_operacao, id_servico, id_tipo, final);
	if (Validar != true) {
		document.getElementById("valor_final" + 1).style.borderColor = 'red';
		$.notify(Validar.mensagem, "warning");
		return false;
...
function Cronograma_fisico(infraestrutura, id_operacao, id_servico, id_tipo, final) {
	if (document.getElementById) {
		var dt = $("#datepicker").datepicker('getDate');
		if (dt.toString() == "Invalid Date") {
			$("#datepicker").datepicker("setDate", new Date());
	...
	$.ajax({
		...
		data: {
			infraestrutura:infraestrutura, id_operacao:id_operacao, id_servico:id_servico, id_tipo:id_tipo, final:final
		},
		dataType: 'json',
		async: false,
		success: function (data) {
