    ...
    $("#insereAtividadesExecutadas").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR ATIVIDADES DA SUPERVISORA]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/AtividadeSupInsereDaq',
                    data: serializedData,
                    dataType: 'json',
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [[EXCLUIR ATIVIDADES DA SUPERVISORA]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgop.php/AtividadeSupExcluirDaq',
                    data: {
                        id_resumo: id_resumo
                    },
                    dataType: 'json',
...
function recuperaAtividadeSupervisora() {
    ...
    $('#atividadesexecutadas').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgop.php/AtividadeSupRetornaDaq",
        "fnServerParams":
                function (aoData) {
                    aoData.push(
...
function editarAtividadeSupervisora(id_resumo) {
    ...
    if (relatorio == 1) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgop.php/AtividadeSupEditarDaq',
...
function btnRecuperaUltimo() {
    ...
    bootbox.confirm("Confirmar operação [Recuperar dados do periodo referência anterior]?", function (result) {
      if (result === true) {
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgop.php/RecuperaSupAtvUltimo?periodo='+termo,
            dataType: 'json',
            success: function (data) {
               if(data.resumo != null){
