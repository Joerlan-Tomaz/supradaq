...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	{
		$DadosServico = $this->Tb_cronogramafinanceiro->RecuperaOperacao();
		$n = 0;
		foreach ($DadosServico as $lista) {
	...
	public function RecuperaServicoOperacao()
	{
		$dados["id_operacao"] = $this->input->post_get("id_operacao");

		$DadosServico = $this->Tb_cronogramafinanceiro->RecuperaServicoOperacao($dados);
		$n = 0;
		foreach ($DadosServico as $lista) {
			$dados['id_servico'][$n] = $lista->id_servico;
			$dados['servico'][$n] = str_replace("_", " ", $lista->servico);
	...
	public function RecuperaTipoOperacao()
	{
		$dados["id_servico"] = $this->input->post_get("id_servico");

		$DadosServico = $this->Tb_cronogramafinanceiro->RecuperaTipoOperacao($dados);
		$n = 0;
		foreach ($DadosServico as $lista) {
			$dados['id_tipo'][$n] = $lista->id_tipo;
