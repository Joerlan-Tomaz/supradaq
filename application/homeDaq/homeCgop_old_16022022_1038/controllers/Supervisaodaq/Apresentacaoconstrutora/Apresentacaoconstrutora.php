<?php
/*
 * Classe controller Apresentacaoconstrutora. 
...
class Apresentacaoconstrutora extends CI_Controller {
...
//-------------------------------------------Recupera-----------------------------------------
     ...
     public function RecuperaApresentacaoConstrutora(){
        $dados["id_contrato_obra"] = $this->session->idContrato;
        $dados["periodo"] = $this->input->post_get("periodo");

        $dadosContrato = $this->Tb_apresentacao_construtora->RecuperaApresentacaoConstrutora($dados);
        foreach ($dadosContrato as $lista) {
            
            $dados["data_base"] = $lista->data_base;
            $dados["contrato"] = $lista->contrato;
            $dados["empresa"] = $lista->nome_empresa;
...
//-------------------------------------------Insere-----------------------------------------
    ...
    public function insereApresentacaoconstrutora() {
        ...
        if (empty($dados["id_apresentacao"])) {
            $retorno = $this->Tb_apresentacao_construtora->insereApresentacaoconstrutora($dados);
        }
    ...
    public function recuperaPortariasFiscais(){
        ...
        $Dados = $this->Tb_apresentacao_construtora->recuperaPortariasFiscais($dados);
        $retorno["data"] = Array();
...
public function recuperaSelectHidrovia(){
	$hidrovias = $this->Tb_apresentacao_construtora->recuperaSelectHidrovia();
	echo (json_encode($hidrovias));
}
