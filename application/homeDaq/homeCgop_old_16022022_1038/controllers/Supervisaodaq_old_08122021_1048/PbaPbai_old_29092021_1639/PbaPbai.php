...
//----------------------------------------INSERE--------------------------------------------
     ...
     //            $dados["desc_pba_pbai"] = $this->input->post_get('resumoPbaPbai');
     //            $dados["pba"] = $this->input->post_get('pba');
     //            $dados["pbai"] = $this->input->post_get('pbai');
                    
     //            $dados["desc_arquivo"] = $this->input->post_get('pba');
                               

     //             $this->Tb_pba_pbai->inserePbaPbai($dados);
 ...
 //------------------------------RECUPERA-----------------------------------------------------
        ...
        if (!empty($Dados)) {
            foreach ($Dados as $lista) {
               ...
               // $arquivo = "<a href='javascript:void(0);' onclick=\"anexoPbaPbai('{$lista->nome_arquivo}')\">" . $nomeArquivo . "</a>";
                ...
                $retorno["data"][] = array(
                    'RESUMO' => $lista->desc_pba_pbai,
                    'ARQUIVO' => $arquivo,
                    'PBA' => $lista->desc_pba,
                    'PBAI' => $lista->desc_pbai,
                    'NOME' => $lista->nome,
...
//------------------------------RECUPERA-EDITAR----------------------------------------------------
    ...
    foreach ($DadosServico as $lista) {
        ...
        $dados['pba'] = $lista->id_pba;
        $dados['pbai'] = $lista->id_pbai;
        $dados['resumo']= $lista->desc_pba_pbai;
