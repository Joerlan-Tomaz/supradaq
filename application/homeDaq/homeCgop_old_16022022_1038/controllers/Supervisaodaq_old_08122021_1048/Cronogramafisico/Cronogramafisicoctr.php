			...
			foreach ($Dados as $lista) {
				...
				if ($lista->publicar <> null) {
					...
					if ($result == 'false') {
						...
						$publicar = "<button type='button' id='publicado_naopublicado_$linha' name='publicado_naopublicado_$linha' class='btn btn-default'  href='javascript:void(0);' onclick='PublicarCronograma_naopublicado(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\")' data-toggle='tooltip' title='Publicar cronograma.' data-placement='top'><i class = 'fas fa-clipboard'></i></buttonx>";
					}
					$detalhar = "<button type='button' id='detalhar_naopublicado_$linha' name='detalhar_naopublicado_$linha' class='btn btn-default' href='javascript:void(0);' onclick='DetalhadoNaoPublicado(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\"," . $linha . "," . $conte . ")'><i class = 'fa fa-eye'></i></buttonx>";
					$inserir = "<button type='button' id='inserir_naopublicado_$linha' name='inserir_naopublicado_$linha' class='btn btn-warning' href='javascript:void(0);' onclick='inserirCronograma(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\")'><i class = 'fas fa-pencil-alt'></i></buttonx>";

				} else {
					$detalhar = "<button type='button' id='detalhar_naopublicado_$linha' name='detalhar_naopublicado_$linha' class='btn btn-default' href='javascript:void(0);' onclick='(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\")'><i class = 'fa fa-eye'></i></buttonx>";
					$inserir = "<button type='button' id='inserir_naopublicado_$linha' name='inserir_naopublicado_$linha' class='btn btn-default' href='javascript:void(0);' onclick='inserirCronograma(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\")'><i class = 'fas fa-pencil-alt'></i></buttonx>";
...
//-------------------------------------------------------------------------------------------------------------------------
	public function CronogramaPublicado()
	{
		...
		if (!empty($Dados)) {
			$linha = 1;
			foreach ($Dados as $lista) {
				$detalhar = "<button type='button' id='detalhar_ $linha' name='detalhar_ $linha' class='btn btn-default' href='javascript:void(0);' onclick='DetalhadoPublicado(" . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\"," . $linha . "," . $conte . ")'><i class = 'fa fa-eye'></i></buttonx>";
...
//--------------------------------------------------------------------------------------------------------------------------
	public function Validar_Porcentagem()
	{
		...
		$dados["nome_eixo"] = $this->input->post_get("nome_eixo");
		$dados["versao"] = $this->input->post_get("versao");
...
//-------------------------------------------------------------------------------------------------------------------------
	public function insereCronogramaFisico()
	{
		...
		if (!empty($DadosCronogramaFisicoVersao)) {
			...
			$this->Tb_cronograma_fisico->excluirItens($dados);
			for ($j = 1; $j <= 12; $j++) {
	...
	public function insereCronogramaFisicoNovo()
	{
		...
		if (!empty($DadosCronogramaFisicoVersao)) {
			...
			if ($dados["publicar_cronograma"] == 'S') {
				...
				$this->Tb_cronograma_fisico->insereVersao($dados);
			}
	...
	public function ServicosInseridosAquaviario()
	{
		...
		if (!empty($DadosItensInseridos)) {
			...
			foreach ($DadosItensInseridos as $lista) {
				...
				$excluir = "<button type='button' class='btn btn-default' href='javascript:void(0);' onclick='excluirItem(" . $lista->id_cronograma_fisico . "," . $lista->id_cronograma . "," . $lista->versao . ",\"" . $lista->nome_eixo . "\");' data-toggle='tooltip' title='Excluir' data-placement='top'><i class='fa fa-trash'></i></button>";
				if ($lista->tipo_obra == '') {
					$lista->tipo_obra = "--";
				}
...
//-------------------------------------------------------------------------------------------------------------------------
	public function RecuperaGeorreferenciamento()
	{
		...
		foreach ($DadosRecuperaGeo as $lista) {
			$retorno["data"][] = array(
				$dados['nome_eixo'] = $lista->nome_eixo,
...
//-------------------------------------------------------------------------------------------------------------------------
	public function DetalhadoPublicado()
	{
		...
		$dados["idUsuario"] = $this->session->id_usuario_daq;
		$dados["id_cronograma"] = $this->input->post_get('id_cronograma');
		$dados["nome_eixo"] = $this->input->post_get('id_operacao');
		$dados["versao"] = $this->input->post_get('versao');
		...
		if (!empty($DadosItensInseridos)) {
			$linha = 1;
			foreach ($DadosItensInseridos as $i => $lista) {
				if ($lista->tipo == '') {
					$lista->tipo = "--";
				}

				$executora_sim_nao = "--";
				$medida = number_format($lista->valor_medido, 2, ",", ".") . '%';
				...
				if($i == count($DadosItensInseridos) - 1){
					$dados["id_tela_formulario"] = '10';
					$dados["periodo"] = $this->input->post_get("periodo");
...
//-------------------------------------------------------------------------------------------------------------------------
	...
	public function RetornaServiçosPreencher()
	{
		$dados["id_contrato_obra"] = $this->session->idContrato;
		$dados["idUsuario"] = $this->session->id_usuario_daq;
		$dados["servico"] = $this->input->post_get('servico');
		$dados["tipo"] = $this->input->post_get('tipo');
		$dados["valanoreferente"] = $this->input->post_get('valanoreferente');
		$dados["nome_eixo"] = $this->input->post_get('nome_eixo');

		$DadosItensInseridos = $this->Tb_cronograma_fisico->ServicosInseridosAquaviario($dados);
		foreach ($DadosItensInseridos as $i => $dadosItens) {
			$DadosItensInseridos[$i]->vm = number_format($dadosItens->vm, 2, ",", ".");
		}
		echo(json_encode($DadosItensInseridos));
