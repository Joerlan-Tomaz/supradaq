...
class EnsaiosLaboratoriais extends CI_Controller
{
	...
	public function insereEnsaioLaboratorio()
	{
		if (isset($_FILES['arquivo'])) {
			...
			if ($maxsize > $arquivo["size"]) {
				...
				if (($extensao === "pdf") || ($extensao === "xlsx") || ($extensao === "docx") || ($extensao === "PDF") || ($extensao === "XLSX") || ($extensao === "DOCX")) {
					...
					$dados["resumo"] = $this->input->post_get('resumoEnsaioLaboratorio');
					...
					$dados["desc_arquivo"] = "EnsaioConstrucao";
...
//------------------------------------------------------------------------------------------------------------------------
	public function excluirEnsaioSup()
	{
	...
	public function anexoEnsaiosup()
	{
		$nome_arquivo = $this->input->post_get('nome_arquivo');
	...
	public function recuperaEnsaiosLaboratorio()
	{
		$dados["idContrato"] = $this->session->idContrato;
		$dados["periodo"] = $this->input->post_get("periodo");
		$dados["roteiro"] = "7";
		...
		if (!empty($Dados)) {
			foreach ($Dados as $i => $lista) {
				...
				//$arquivo = "<a href='javascript:void(0);' onclick=\"anexoEnsaiosup('{$lista->nome_arquivo}')\">" . $nomeArquivo . "</a>";
