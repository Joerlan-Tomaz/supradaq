...
class OcorrenciaProjeto extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->library("session");
        $this->load->model("/Supervisaodaq/Tb_ocorrencia_projeto");
      ...
      public function insereArquivoOcorrencia() {
        if (isset($_FILES["arquivo"])) {
            ...
            if ($maxsize > $arquivo["size"]) {
                ...
                if (($extensao === "xlsx")||($extensao === "XLSX")) {
                    ...
                    $dados["desc_arquivo"] = "Ocorrencia_Projeto";
...
//------------------------------------------------------------------------------------------------------------------------
    public function excluirOcorrecia() {
     ...
     public function recuperaOcorrenciaProjeto() {
        $dados["id_contrato"] = $this->session->idContrato;
        //$dados["periodo"] = $this->input->post_get("periodo");
        $dados["data"] = array();
        $dadosGeo = $this->Tb_ocorrencia_projeto->recuperaOcorrenciaProjeto($dados);
        ...
        if (!empty($dadosGeo)) {
            foreach ($dadosGeo as $lista) {
                ...
                $detalhes = "<button type='button' class='btn btn-default' href='javascript:void(0);' onclick=\"detalhesOcorrenciaProjeto({$lista->id_arquivo})\"><i class = 'fa fa-eye'></i></button>";
                //$arquivo = "<a href='javascript:void(0);' onclick=\"anexoProjeto('{$lista->nome_arquivo}')\">" . $nomeArquivo . "</a>";
                ...
                $dados["data"][] = array(
                    ...
                    "ACAO" => "<a data-toggle='tooltip' title='Excluir' data-placement='top' class='btn btn-sm btn-default' href='javascript:void(0);' onclick=\"excluirOcorrenciaProjeto({$lista->id_arquivo},'{$nomeArquivo}')\">
    ...
    public function ModeloOcorrenciaProjeto() {
        $nome_arquivo = $this->input->post_get('nome_arquivo');
        $config['hostname'] = '10.100.11.158';
        $config['username'] = 'supradaq';
        $config['password'] = 'dnit@2020';
      ...
      public function recuperaDetalhesOcorrenciaProjeto() {
        $dados["id_arquivo"] = $this->input->post_get("id_arquivo");
        $dados["data"] = array();
        $dadosGeo = $this->Tb_ocorrencia_projeto->detalhesOcorrenciaProjeto($dados);
      ...
      public function excluirArquivo() {
        ...
        if ($retorno == true) {
            $retorno = $this->Tb_ocorrencia_projeto->excluirOcorrenciaProjeto($dados);
        }
    ...
    public function inserirdados(){
        ...
        //-------------------------------------------------------------------------------------
        $a = 'Nome '; $b = 'Tipo'; $c = 'ESTACA'; $d = 'FRAÇÃO DE ESTACA'; $e = 'Km';
        $f = 'LATITUDE'; $g = 'LONGITUDE';
        //-------------------------------------------------------------------------------------
        ...
        if ($verificaArray == 6)
            continue;
               ...
               $dados[]=array(
                        ...
                        'id_usuario' => $this->session->id_usuario_daq,        
                        'nome'=>$val ['1'],
                        'tipo'=>$val ['2'],        
        ...
        if($inseridos == 0){
          ...
          $retorno = $this->Tb_ocorrencia_projeto->inserirdados($dados);
