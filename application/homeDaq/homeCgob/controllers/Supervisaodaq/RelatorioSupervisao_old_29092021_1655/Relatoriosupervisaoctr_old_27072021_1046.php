...
//-----------------------------------------------------------------
	public function validarInserir()
	{
		...
		//$dados["idperfil"] = $this->session->id_perfil_daq;
                #--------------------------------------------------------------#
	...
	public function recuperaContrato()
	{
		...
		if (!empty($DadosAnaliseRelatorio)) {
			foreach ($DadosAnaliseRelatorio as $lista) {
				...
				//------------------------------------------------------------------------------
				if (($relatorio == "") or ($aceite == "aguardando análise" && $roteiro_analise == "fechar_relatorio" && ($perfil == 1 OR $perfil == 9) )) {
					$status_versao = "Aguardando Análise - [" . $versao . "]";
				...
				if (($aceite == "em analise" && ($perfil == 3 OR $perfil == 9)) or ($aceite == "em analise" && ($perfil == 2 OR $perfil == 9))) {
					$status_versao = "Em Análise - [" . $versao . "]";
