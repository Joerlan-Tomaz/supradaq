...
$(document).ready(function () {
	$('#table_altera_usuario').dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/painelAdmRecuperaInfoUsuario",
		"aoColumns": [
...
function tabelaContratos() {
	...
	if (tipo != '') {
		$.ajax({
			type: 'POST',
			url: base_url + 'index_cgob.php/painelAdmBuscaContratosUsuario',
			dataType: 'json',
...
function populaSupervisora(id_usuario) {
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/painelAdmPopulaSupervisora',
		dataType: 'json',
		data: {id_usuario: id_usuario},
		async: false,
		success: function (data) {
...
function alteraPerfilPermissao() {
	...
	bootbox.confirm("Confirmar operação [ALTERAR PERFIL/PERMISSÃO]?", function (result) {
		if (result === true) {
			$.ajax({
				type: 'POST',
				url: base_url + "index_cgob.php/painelAdmAlteraPerfilPermissao",
				data: serializedData,
				dataType: 'json',
				success: function (data) {
