...
function Validar_Porcentagem(id_obra, id_servico, versao, final) {
	$.ajax({
		...
		data: {
			id_obra: id_obra, id_servico: id_servico, versao: versao, final: final
		},
		dataType: 'json',
		async: false,
		success: function (data) {
			...
			for (var numero = 1; numero <= 12; numero++) {
				if (!$("#filho" + numero)[0]) {
					$("#campoPai").append(
						...
						"   </div>" +
						"   <div class='col-md-3'>" +
						"           <label for='valor_medido'>" + (inp_user) + " :</label><input class='form-control' type='text' id='valor_medido" + numero + "' name='valor_medido[" + numero + "]' maxlength='9' onkeydown='FormataMoeda(this, 7, event)' onkeypress='return maskKeyPress(event)' value=''>" +
						"   </div>" +
						"   <div class='col-md-3'>" +
						"           <label>&nbsp;</label><input  class='btn btn-block btn-info' type='button' onclick='removerCampo(" + numero + ")' value='Remover'>" +
						"   </div>" +
