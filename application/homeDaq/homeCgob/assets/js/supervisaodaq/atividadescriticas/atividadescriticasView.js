    ...
    $("#insereAtividadesCriticas").click(function () {
        ...
        bootbox.confirm("Confirmar operação [INSERIR ATIVIDADE CRÍTICA]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgob.php/AtividadeCriticaInsereDaq',
                    data: serializedData,
                    dataType: 'json',
    ...
    if (relatorio == 1) {
        ...
        bootbox.confirm("Confirmar operação [EXCLUIR ATIVIDADE CRÍTICA]?", function (result) {
            if (result === true) {
                $.ajax({
                    type: 'POST',
                    url: base_url + 'index_cgob.php/AtividadesCriticasExcluiDaq',
...
function recuperaAtividadesCriticas() {
    ...
    $('#tableAtividadesCriticas').dataTable({
        ...
        "sAjaxSource": base_url + "index_cgob.php/AtividadesCriticasRecuperaDaq",
...
function editarAtividadesCriticas(id_resumo) {
    ...
    if (relatorio == 1) {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgob.php/AtividadesCriticasEditarDaq',
            data: {id_resumo: id_resumo},
            dataType: 'json',
            success: function (data) {
                document.getElementById("datepicker").disabled = true;
...
function btnRecuperaUltimo() {
    ...
    bootbox.confirm("Confirmar operação [Recuperar dados do periodo referência anterior]?", function (result) {
      if (result === true) {
        $.ajax({
            type: 'POST',
            url: base_url + 'index_cgob.php/RecuperaCritUltimo?periodo='+termo,
            dataType: 'json',
            success: function (data) {
               if(data.resumo != null){
