...
function recuperaAnaliseContrato() {
	...
	$('#tableAnaliseContrato').dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/AnaliseContratoDaq?uf=" + uf,
...
function iniciarAnalise(id_contrato) {
	...
	bootbox.confirm("Adicionar relátorio para análise?", function (result) {
		if (result == true) {
			...
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgob.php/AnaliseConfirmarDaq?periodo=' + termo + '&id_contrato=' + id_contrato,
				dataType: 'json',
...
function ordemperfil(id_contrato) {
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/AnalisePerfilDaq',
		data: {
...
function dadosContrato(id_contrato) {
	...
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/AnaliseVersaoDaq',
		data: {
...
function dadosRelatorio(id_contrato) {
	...
	$('#table_modulos_analise').dataTable({
		...
		"bLengthChange": false,
		"paging": false,
		"sAjaxSource": base_url + "index_cgob.php/AnaliseModulosDaq?id_contrato=" + id_contrato,
		"fnServerParams": function (aoData) {
			aoData.push(
...
function EditarAnalise(id_modulo, modulo, id_contrato) {
	...
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/AnaliseEditarDaq',
...
function insereAceite() {
	...
	bootbox.confirm("Confirma cadastrado [ Retificar ]?", function (result) {
		if (result == true) {
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgob.php/AnaliseInsereEditarDaq',
				data: serializedData,
				dataType: 'json',
				success: function (data) {
...
function confereAprovar(id_contrato) {
	...
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/ConfereAnaliseDaq',
		data: {
			periodo: termo, id_contrato: id_contrato
		},
		dataType: 'json',
...
function ModalhistoricoAnalise() {
	...
	$('#tableHistoricoAnalises').dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/AnaliseHistoricoDaq?id_contrato=" + id,
		"fnServerParams": function (aoData) {
			aoData.push(
				{"name": "periodo", "value": termo}
			);
...
function insereFinalizar() {
	...
	bootbox.confirm("Confirmar operação [CONCLUIR ANÁLISE]?", function (result) {
		if (result === true) {
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgob.php/AnaliseConclusaoDaq',
				data: serializedData,
				dataType: 'json',
				success: function (data) {
...
function retornaRelatorio(id_contrato) {
	...
	jan = window.open(base_url + 'index_cgob.php/ImpressaoRelatorioDaq?periodo=' + termo + '&id=' + id, 'jan', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=100, LEFT=left, WIDTH=800, HEIGHT=500');
	...
	if (id == '') {
		...
		bootbox.confirm("Confirmar operação [EXCLUIR NÃO ACEITE]?", function (result) {
			if (result === true) {
				$.ajax({
					type: 'POST',
					url: base_url + 'index_cgob.php/AnaliseExcluirDaq',
					data: {
						id: id
					},
					dataType: 'json',
