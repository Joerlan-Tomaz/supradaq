...
//---------------- Validação de formulario -----------------------------
    ...
    $("#fileOcorrenciaProjeto_planilha_old").change(function () {
        ...
        $.ajax({
            type: "POST",
            url: base_url + "index_cgob.php/OcorrenciaProjetoContagemDaq",
            data: form,
            contentType: false,
            processData: false,
            dataType: "json",
...
//-----------------------------------------------------------------------
    ...
    $("#tableOcorrenciaProjeto").dataTable({
        ...
        "sAjaxSource": base_url + "index_cgob.php/OcorrenciaProjetoRecuperaDaq?periodo="+termo,
	...
	if (relatorio == 1) {
		...
		bootbox.confirm("Confirmar operação [EXCLUIR REGISTROS DE OCORRÊNCIA DE PROJETO DO ARQUIVO  '" + arquivo + "']?", function (result) {
			if (result === true) {
				$.ajax({
					type: 'POST',
					url: base_url + 'index_cgob.php/OcorrenciaProjetoExcluirDaq',
...
function detalhesOcorrenciaProjeto(id_arquivo) {
    ...
    $("#tableDetalhesOcorrenciaProjeto").dataTable({
        ...
        "sAjaxSource": base_url + "index_cgob.php/OcorrenciaProjetoDetalhesDaq",
        "fnServerParams": function (aoData) {
     ...
     //------------------ Verificação de campos -----------------------------
      ...
      //  bootbox.confirm("Confirmar operação [INSERIR GEORREFERENCIAMENTO]?", function (result) {
          //  if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgob.php/OcorrenciaProjetoInsereDaq",
                    dataType: "json",
                    data: form,
                    dataType: "json",
                    contentType: false,
    ...
    //------------------ Verificação de campos -----------------------------
       ...
       // bootbox.confirm("Confirmar operação [INSERIR GEORREFERENCIAMENTO]?", function (result) {
          //  if (result === true) {
                $.ajax({
                    type: "POST",
                    url: base_url + "index_cgob.php/OcorrenciaProjetoInsereDadosDaq?nomeArquivo="+nomeArquivo+"&idArquivo="+idArquivo,
                    dataType: "json",
                    data: form,
                    dataType: "json",
                    contentType: false,
...
function excluirOcorrecia(nome_arquivo) {
    $.ajax({
        url: base_url + 'index_cgob.php/excluirOcorrecia',
        type: 'POST',
        data: {nome_arquivo: nome_arquivo},
        success: function (data) {
