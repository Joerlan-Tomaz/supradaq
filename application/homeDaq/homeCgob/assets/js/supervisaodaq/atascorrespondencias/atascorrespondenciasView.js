	...
	$("#insereAtasCorrespondencias").click(function () {
		...
		bootbox.confirm("Confirmar operação [INSERIR ATAS E CORRESPONDÊNCIAS]?", function (result) {
			...
			if (result === true) {
				$.ajax({
					type: 'POST',
					url: base_url + 'index_cgob.php/AtasCorrespondenciaInseredaq',
					data: form,
					dataType: 'json',
					contentType: false,
					processData: false,
...
function recuperaAtaCorrespondencia() {
	...
	$('#tableAtasCorrespondencias').dataTable({
		...
		"sAjaxSource": base_url + "index_cgob.php/retornaAtasCorrespondenciasDaq?periodo=" + termo,
...
function table_naohouveatividademes() {
	...
	$.ajax({
		type: 'POST',
		url: base_url + 'index_cgob.php/AtasNaoAtividadeDaq',
		data: {periodo: termo},
		dataType: 'json',
		success: function (data) {
			if (data.atas == true) {
				...
				$('#tableNaohouveAtividadeMes').dataTable({
					...
					"sAjaxSource": base_url + "index_cgob.php/AtasNaoAtividadeDaq?periodo="+termo,
					"aoColumns": [
						{data: 'atividademes'},
						{data: 'usuario'},
...
function btnNoAtividade() {
	...
	bootbox.confirm("Confirmar operação [NÃO HOUVE ATIVIDADE NO MÊS]?", function (result) {
		if (result === true) {
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgob.php/AtasInsereNaoAtividadeDaq',
				data: {
					periodo: termo
				},
				dataType: 'json',
...
function excluirNaoAtividade(id) {
	bootbox.confirm("Confirmar operação [EXCLUIR]?", function (result) {
		if (result === true) {
			$.ajax({
				type: 'POST',
				url: base_url + 'index_cgob.php/AtasExcluirNaoAtividadeDaq?id=' + id,
				dataType: 'json',
				success: function (data) {
					table_naohouveatividademes();
