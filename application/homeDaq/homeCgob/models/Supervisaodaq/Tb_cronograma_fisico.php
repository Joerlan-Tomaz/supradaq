...
//-----------------------------------------------------------------------------------------------------------------
	public function acompanhementofisico($dados){
		...
		with previsto as (
	 	...
	 	'p' as identit, 
                (SELECT od.desc_obra FROM CGOB_TB_OBRA_DERROCAMENTO AS od WHERE cf.id_servico = od.id_obra) servico,
                cf.unidade_medida,
		cf.ano,
		cf.mes,
		'2015-01-01' as maior,
		...
		union all
			...
			af.obra as obra, 
			'c' as identit, 
                        (SELECT od.desc_obra FROM CGOB_TB_OBRA_DERROCAMENTO AS od WHERE af.servico = od.id_obra) servico,
                        af.unidade_medida,
...
//-----------------------------------------------------------------------------------------------------------------
	...
	public function excluirItens($dados){
        ...
        $this->db->where("id_obra", $dados['id_obra']);
        $this->db->where("id_servico", $dados['id_servico']);
        $this->db->where("tipo_obra", $dados['tipo']);
        $this->db->set("publicar", "N");
        $this->db->set("data_publicar", date("Y-m-d H:i:s"));
