...
//-------------------------------------------------------------------------------------------------------
	public function recuperaDados($dados)
	{
        ...
        INNER JOIN TB_USUARIO as us on an.id_usuario = us.id_usuario
              WHERE id_relatorio_edit = (SELECT MIN(id_relatorio_edit) FROM CGOB_TB_HISTORICO_RELATORIO WHERE id_contrato_obra = '" . $dados["id_contrato"] . "' AND id_aceite in ('aguardando análise') AND perfil in ('1','9')) 
      ";
...
//-------------------------------------------------------------------------------------------------------
	public function validar($dados)
	{
        ...
        FROM CGOB_TB_HISTORICO_RELATORIO
              WHERE id_contrato_obra = '" . $dados["id"] . "' AND perfil in ('2','9')
      ";
...
//-------------------------------------------------------------------------------------------------------
	public function returnTecnicodaq($dados)
	{
        ...
        CONCAT(CONVERT(CHAR(10), ad.ultima_alteracao , 103),' ', CONVERT(CHAR(8), ad.ultima_alteracao , 114)) AS ultima_alteracao  
        FROM CGOB_TB_HISTORICO_RELATORIO as ad
            WHERE ad.id_contrato_obra = '" . $dados["id_contrato"] . "' AND periodo_referencia = '" . $dados["periodo"] . "' AND perfil in ('3')
      ";
