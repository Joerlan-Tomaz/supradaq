...
//----------------------------------------------------------------------------------------------------------------------------
	public function RecuperaMedicaoNaopublicado($dados)
	{
		...
		round(ma.valor_pi_medicao,2)valor_pi_medicao, 
		(SELECT round(SUM(valor),2) FROM CGOB_TB_AVANCO_FINANCEIRO 
		...
		INNER JOIN CGOB_TB_CONTRATO_OBRA AS c ON c.nu_con_formatado = ma.contrato 
		WHERE c.id_contrato_obra = " . $dados["id_contrato_obra"] . "
                and (SELECT count(publicar_versao) FROM CGOB_TB_AVANCO_FINANCEIRO 
		        WHERE id_contrato_obra = " . $dados["id_contrato_obra"] . " 
		        AND (publicar ='S' and publicar_versao = 'S') and nume_medicao = ma.nume_medicao  )=0    
		";
