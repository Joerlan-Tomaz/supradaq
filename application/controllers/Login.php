...
class Login extends CI_Controller {
    ...
    function __construct() {
       ...
       // $this->load->model('homeCgcont/Supervisaocont/Mensagem/Msg_entrada');
       // $this->load->model('homeCgcont/Supervisaocont/Mensagem/Msg_saida');
       // $this->load->model('homeCgcont/Supervisaocont/Mensagem/Msg_resposta');
    ...
    public function validar_login() {
        ...
        $retorno = $this->Tb_usuario->validar_login($email, md5($senha)); //die($retorno);
        // $retorno = json_decode($retorno, true);
        if (!empty($retorno)) {
            foreach ($retorno as $r) {
                $ipUsuario = getenv("REMOTE_ADDR");
                $desc_nome = $r->DESC_NOME;
                $id_perfil = $r->ID_PERFIL;
                $id_perfil_cgmrr = $r->ID_PERFIL_CGMRR;
                $id_perfil_cgpert = $r->ID_PERFIL_CGPERT;
                $coordenacao = $r->coordenacao;
                $email = $r->email;
                $id_usuario = $r->ID_USUARIO;
                $dt_Ultacesso = $r->DATA_ULTIMOACESSO;
                $boAlteraSenha = $r->FLAG_ALTERASENHA;
                $stPerfilFecharRelatorio = $r->fechar_relatorio;
                $empresa = $r->empresa;
                $id_uf = $r->id_uf;
                $id_organizador = $r->id_organizador;
                $id_usuario_daq = $r->id_usuario_daq;
                $id_usuario_dif = $r->id_usuario_dif;
                $id_usuario_daf = $r->id_usuario_daf;
                $id_usuario_direx = $r->id_usuario_direx;
                $id_usuario_dpp = $r->id_usuario_dpp;
                $analisar_relatorio = $r->analisar_relatorio;
                if(array_key_exists("id_gestao_demandas",$r)){
                    $id_gestao_demandas = $r->id_gestao_demandas;
                }
                if(array_key_exists("id_imr",$r)){
                    $id_imr = $r->id_imr;
                }
            }
        } else {
        ...
        if (empty($id_usuario) or empty($id_perfil) or empty($email)) {
            $retorno = 3218181;
        } else {
            $log = array(
                'desc_nome' => $desc_nome,
                'id_perfil' => $id_perfil,
                'id_perfil_cgmrr' => $id_perfil_cgmrr,
                'id_perfil_cgpert' => $id_perfil_cgpert,
                'email' => $email,
                'id_usuario' => $id_usuario,
                'dt_Ultacesso' => $dt_Ultacesso,
                'boAlteraSenha' => $boAlteraSenha,
                'stPerfilFecharRelatorio' => $stPerfilFecharRelatorio,
                'empresa' => $empresa,
                'id_uf' => $id_uf,
                'alteraSenha' => $boAlteraSenha,
                'coordenacao' => $coordenacao,
                'id_organizador' => $id_organizador,
                'id_usuario_daq' => $id_usuario,
                'id_usuario_dif' => $id_usuario_dif,
                'id_usuario_daf' => $id_usuario_daf,
                'id_usuario_direx' => $id_usuario_direx,
                'id_usuario_dpp' => $id_usuario_dpp,
                'analisar_relatorio' => $analisar_relatorio,
                'id_gestao_demandas' => $id_gestao_demandas,
                'id_imr' => $id_imr
            );
     ...
     //   }
        ...
        if (!empty($Dados)) {
            ...
            foreach ($Dados as $lista) {
                $filename = "assets/img/users/". $lista->id_upload;
                if (file_exists($filename)) {
                    echo "<span style='position: relative;  font-size: 9px; padding: 2px 3px;  background-color: #00a65a !important;   border-radius: .25em;  left: 50px;bottom: 15px;'>" . $num . "</span> <img src='" . base_url("assets/img/users/") . $lista->id_upload . "' alt='' style='width: 40px; border-radius: 50%;height: 40px;'>";
                }
                else{
                   echo "<span style='position: relative;  font-size: 9px; padding: 2px 3px;  background-color: #00a65a !important;   border-radius: .25em;  left: 50px;bottom: 15px;'>" . $num . "</span> <img src='" . base_url("assets/img/users/default_user.png") . "' alt='' style='width: 40px; border-radius: 50%;height: 40px;'>";
                }
            }
        } else {
            echo "<span style='position: relative;  font-size: 9px; padding: 2px 3px;  background-color: #00a65a !important;   border-radius: .25em;  left: 50px;bottom: 15px;'>" . $num . "</span> <img src='" . base_url("assets/img/users/default_user.png") . "' alt='' style='width: 40px; border-radius: 50%;height: 40px;'>";
    ...
    public function alteraSobreMim() {
        ...
        if (isset($_FILES['arquivo'])) {
            ...
            if ($maxsize > $arquivo["size"]) {
                ...
                if (($extensao == "jpg") || ($extensao == "jpeg") || ($extensao == "JPG") || ($extensao == "png")) {
                    ...
                    $retorno["mensagem"] = "Erro. Extensão da imagem não permitida! (Permitidas jpg, jpeg e png. )";
                    $retorno["notify"] = "warning";
                    echo (json_encode($retorno));
                    exit();
                }               
            }else{
                $retorno["mensagem"] = "Erro. Tamanho da imagem não permitida!";
                $retorno["notify"] = "warning";
                echo (json_encode($retorno));
                exit();
            }
