...
class Home extends CI_Controller
{
    ...
    public function home()
    {
        ...
        $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil, 'idUsuario' => $this->session->id_usuario,'id_usuario' => $this->session->id_usuario,'analisar_relatorio' => $this->session->analisar_relatorio);
        ...
        foreach ($retorno as $lista)
        {
            if ($lista->FLAG_ALTERASENHA == "S")
            {
                ...
                } else if (strtotime($prazoSenha) > strtotime(date('Y-m-d H:i:s')))
                {
                    ...
                    //$data = array('Codigo' => "1", 'CodigoAviso' => 1, 'MensagemAviso' => $mensagem, 'IdPerfil' => $this->session->id_perfil,'analisar_relatorio' => $this->session->analisar_relatorio);
                    $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil, 'idUsuario' => $this->session->id_usuario,'id_usuario' => $this->session->id_usuario,'analisar_relatorio' => $this->session->analisar_relatorio);
                }
            } else if ($lista->FLAG_ALTERASENHA == "N")
            {
                ...
                } else if (strtotime($prazoSenha) > strtotime(date('Y-m-d H:i:s')) && strtotime($avisoSenha) < strtotime(date('Y-m-d H:i:s')))
                {
                    ...
                    //$data = array('Codigo' => "0", 'CodigoAviso' => 1, 'MensagemAviso' => $mensagem, 'IdPerfil' => $this->session->id_perfil,'analisar_relatorio' => $this->session->analisar_relatorio);
                    $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil, 'idUsuario' => $this->session->id_usuario,'id_usuario' => $this->session->id_usuario,'analisar_relatorio' => $this->session->analisar_relatorio);
                }
            }
        }
        $qntPerfisDiretorias = (!empty($this->session->id_perfil)) + (!empty($this->session->id_usuario_daq)) + (!empty($this->session->id_usuario_dif)) + (!empty($this->session->id_usuario_daf)) + (!empty($this->session->id_usuario_direx)) + (!empty($this->session->id_usuario_dpp));

        if ($qntPerfisDiretorias >= 2)
        {
            $this->load->view("homeDiretorias", $data);
        } else
        {
            ...
            } else
            {
                ...
                {
                    $this->load->model("homeCgcont/GestaoDemandas/Gd_usuario");

                    $dados["id_usuario_responsavel"] = $this->session->id_usuario;

                    $dadosPerfilGD = $this->Gd_usuario->recupera_usuario($dados);

                    if (!empty($dadosPerfilGD))
                    {
                        foreach ($dadosPerfilGD as $p)
                        {
                            $id_gd_usuario = $p->id_gd_usuario;
                        }
                        $log = array(
                            'id_gd_usuario' => $id_gd_usuario,
                        );
                        $this->session->set_userdata($log);
                    ...
                    if (empty($dadosPerfilGD))
                    {
                        $mensagem = "É necessário solicitar o cadastro no gestão de demandas.";
    ...
    public function home_Dir()
    {
        ...
        $retorno = $this->Tb_usuario->recuperaUsuarioSessao();
        $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil, 'idUsuario' => $this->session->id_usuario);
    ...
    public function homeAssessoria()
    {
        $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil);
        $data["idUsuario"] = $this->session->id_usuario;
        $data["id_usuario"] = $this->session->id_usuario;
        $this->load->view('homeAssessoria', $data);
    }

    public function homeCgcont()
    {
        $data = array('Codigo' => "0", 'CodigoAviso' => "0", 'IdPerfil' => $this->session->id_perfil,'analisar_relatorio' => $this->session->analisar_relatorio);
        $data["idUsuario"] = $this->session->id_usuario;
        $data["id_usuario"] = $this->session->id_usuario;
        $this->load->view('homeCgcont/homeCGcont', $data);
    }
