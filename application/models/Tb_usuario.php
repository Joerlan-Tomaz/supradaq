...
class Tb_usuario extends CI_Model {
    ...
    public function validar_login($email, $senha) {
		...
		$SQL_old = "
            SELECT 
                ...
                u.id_usuario_direx,
                id_usuario_dpp,
                u.id_gestao_demandas,
                u.id_imr,
                case 
                when (select publicar from tb_usuario_permissao where id_usuario = u.id_usuario and id_roteiro=60) is NULL then 'N' 
                when (select publicar from tb_usuario_permissao where id_usuario = u.id_usuario and id_roteiro=60) = 'N' then 'N'
                when (select publicar from tb_usuario_permissao where id_usuario = u.id_usuario and id_roteiro=60) = 'S' then 'S'
    ...
    public function alteraUltimoAcesso() {
        ...
        $this->db->set("DATA_ULTIMOACESSO", date("Y-m-d\TH:i:s"));
        $this->db->update("TB_USUARIO");
        return true;
    }
    ...
    public function RecuperaacessoUsuarios() {
        ...
        $SQL .= " ORDER BY data_ultimoacesso DESC";
